...
pub fn day4a() {
    ...
    for (id, list) in guards {
        let max_min_day = list.iter().max().unwrap();
        let max_day = list.iter().position(|&x| x == *max_min_day).unwrap();
        let sleep = list.iter().sum();
        if std::cmp::max(max_sleep, sleep) == sleep {
            max_sleep = sleep;
            max_sleep_id = id;
        }
        mins.insert(id, max_day as i32);
    }
    println!("{}", mins[&max_sleep_id] * max_sleep_id);
}
